import numpy as np
from nn.util import initializer
import math
...
class Variable(object):
    ...
    def __init__(self, shape: list, name: str, scope='', grad: bool = True, learnable: bool = False, init: str = 'MSRA'):
        if scope != '':
    ...
    def apply_gradient(self, learning_rate: float, decay_rate: float, batch_size: int = 1):
        ...
        if self.method == 'SGD':
            ...
            self.mtmp = self.momentum * self.mtmp + self.diff / batch_size + self.momentum * (self.diff - self.last_diff) / batch_size
    ...
    def set_method_momentum(self, momentum: int = 0.9):
        self.method = 'Momentum'
        self.momentum = momentum
        self.mtmp = np.zeros(self.diff.shape)

    def set_method_adam(self, beta1: float = 0.9, beta2: float = 0.999, epsilon: float = 1e-8):
        self.method = 'Adam'
        self.beta1 = beta1
        self.beta2 = beta2
...
def get_by_name(name: str):
    if 'GLOBAL_VARIABLE_SCOPE' in globals():
        try:
            ...
            return Exception("GLOBAL_VARIABLE_SCOPE not include name: %s" % name)
    else:
        return Exception("No Variable")
